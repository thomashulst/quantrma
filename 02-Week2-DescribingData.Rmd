...
>Take home: we can visualize our variables by putting them in a plot, which is easier and more helpful than looking at the raw numbers.
...
The $\sum$ symbol is called **sigma**, and it stands for the operation of summing. The little "i" on the bottom, and the little "n" on the top refers to all of the numbers in the set, from the first number "i" to the last number "n". The $x_{i}$ refers to individual numbers in the set. We sum up all of the numbers, then divide the sum by $N$, which is the total number of numbers. Sometimes you will see $\bar{X}$ (say: ex-bar) to refer to the mean of a set of numbers.
...
ggplot(df_hap, aes(x = happiness)) + geom_histogram(binwidth = 300,color="black", fill="grey") +
...
ggplot(df_call, aes(x = call_length)) + geom_histogram(binwidth = 1,color="black", fill="grey") +
...
Perhaps you were thinking that the problem of summing differences scores (so that they don't equal zero), can be solved in more than one way. How about just taking the absolute value of the difference scores? [^Abs] 
...
That's it for the readings of the second week. During the lab you will learn about graphing and generating descriptive statistics in R.
